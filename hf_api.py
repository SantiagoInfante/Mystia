...
# SOLUCIÓN: Inicializar 'client' a None antes del bloque try/except
client = None

# Inicializa el cliente de Hugging Face
try:
    HF_TOKEN = os.environ.get("HF_TOKEN")
    # Inicialización del cliente con token y timeout
...
def query_hf(prompt, model_id):
    ...
    """
    # 'client' está garantizado de existir aquí
    if client is None:
    ...
    try:
        ...
        if "StopIteration" in str(e) or "provider" in str(e):
            return "Error al conectar con el servidor del modelo. El modelo puede no ser compatible con la API de inferencia gratuita."
